...
function generateAdvisorSheets(
  ...
  reports.forEach((report) => {
    ...
    advisorGroups.get(advisor)!.push({
      ...
      '4.准备工作': (report.contacted_professor && report.professor_replied) ? (report.preparation_work || '') : '',
      '5.问题1': (report.contacted_professor && report.professor_replied && report.question_list) ? (report.question_list.split('\n')[0] || '') : '',
      '6.问题2': (report.contacted_professor && report.professor_replied && report.question_list) ? (report.question_list.split('\n')[1] || '') : '',
      '7.问题3': (report.contacted_professor && report.professor_replied && report.question_list) ? (report.question_list.split('\n')[2] || '') : '',
    ...
    const excelData = reports
      ...
      .map(report => {
        ...
        return {
          ...
          '3.导师是否回复？': report.contacted_professor ? (report.professor_replied ? '是' : '否') : '',
          '4.准备工作': (report.contacted_professor && report.professor_replied) ? (report.preparation_work || '') : '',
          '5.问题1': (report.contacted_professor && report.professor_replied && questions.length > 0) ? (questions[0] || '') : '',
          '6.问题2': (report.contacted_professor && report.professor_replied && questions.length > 1) ? (questions[1] || '') : '',
          '7.问题3': (report.contacted_professor && report.professor_replied && questions.length > 2) ? (questions[2] || '') : '',
          '8.导师反馈': (report.contacted_professor && report.professor_replied) ? (report.advisor_feedback || '') : '',
          '9.后续计划': (report.contacted_professor && report.professor_replied) ? (report.follow_up_plan || '') : '',
        }
      })
