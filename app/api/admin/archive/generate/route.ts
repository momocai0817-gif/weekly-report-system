...
import { saveArchiveFile } from '@/lib/storage'
...
async function uploadToStorage(
  ...
  // objectPath 形如 "archives/2026/week-24/unsubmitted.xlsx"
  const m = objectPath.match(/^archives\/(\d+)\/week-(\d+)\/(.+)$/)
  if (!m) throw new Error(`非法 objectPath: ${objectPath}`)
  const [, year, week, filename] = m
  return await saveArchiveFile(Number(year), Number(week), filename, buffer)
}
