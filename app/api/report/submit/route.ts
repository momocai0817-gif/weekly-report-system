...
export async function POST(request: NextRequest) {
  try {
    ...
    // 优先使用 body 里的 week/year（前端从 URL ?week=&year= 读取并显示），
    // 否则回退到 getCurrentWeek()，保证直接 API 调用仍可用。
    let targetWeekNumber: number
    let targetYear: number
    if (
      typeof weekNumber === 'number' && Number.isInteger(weekNumber) && weekNumber > 0 &&
      typeof year === 'number' && Number.isInteger(year) && year >= 2020
    ) {
      targetWeekNumber = weekNumber
      targetYear = year
    } else {
      const currentWeek = getCurrentWeek(now)
      targetWeekNumber = currentWeek.weekNumber
      targetYear = currentWeek.year
    }
    ...
    return NextResponse.json({
      success: true,
      report: data,
      targetWeek: { weekNumber: targetWeekNumber, year: targetYear },
